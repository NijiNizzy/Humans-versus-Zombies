...
public class HumanMovementForces : MovementForces 
{
	// Attributes
	...
	{
		distance = Vector3.Distance (gameObject.transform.position, targetList [0].transform.position);
		int targetIndex = 0;
	...
	override public void UpdatePosition()
	{
		...
		//flee the target if threatened
		if (distance <= safeDistance) 
		{
			Vector3 fleeingForce = Evade (target.transform.position);
